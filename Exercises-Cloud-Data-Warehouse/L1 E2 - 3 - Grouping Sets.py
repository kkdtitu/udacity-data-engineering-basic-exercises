...
get_ipython().run_cell_magic('sql', '', 'SELECT SUM(s.sales_amount) AS revenue\nFROM factSales s;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT st.country, SUM(s.sales_amount) AS revenue\nFROM factSales s\nJOIN dimStore st ON (s.store_key = st.store_key)\nGROUP BY st.country \nORDER BY revenue DESC\nLIMIT 100;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT d.month, SUM(s.sales_amount) AS revenue\nFROM factSales s\nJOIN dimDate d ON s.date_key = d.date_key\nGROUP BY d.month\nORDER BY revenue DESC, d.month ASC\nLIMIT 100;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT d.month, st.country, SUM(s.sales_amount) AS revenue\nFROM factSales s\nJOIN dimDate d ON (s.date_key = d.date_key)\nJOIN dimStore st ON (s.store_key = st.store_key)\nGROUP BY d.month, st.country\nORDER BY d.month ASC, st.country ASC, revenue DESC\nLIMIT 10;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT d.month, st.country, SUM(s.sales_amount) AS revenue\nFROM factSales s\nJOIN dimDate d ON s.date_key = d.date_key\nJOIN dimStore st on s.store_key = st.store_key\nGROUP BY GROUPING SETS ((), (d.month), (st.country), (d.month, st.country))\nORDER BY d.month ASC, st.country ASC, revenue DESC\nLIMIT 50;')
